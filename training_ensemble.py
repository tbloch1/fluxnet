...
for i in range(n_ensembles):
    ...
    model_folder = 'models/{0:05d}_ensemble/model_{1}/'.format(len(glob.glob(savepath+'*')),i)
    ...
    for epoch in pbar:
        ...
        if epoch%100 == 0:
            testpred = net(Xtest.float())
            testloss = (loss_func(testpred.float(), ytest.float())/ytest)
